...
// a dead signal; NETWORK-ONLY for everything cross-origin.
//
// That second rule is the important one and is the opposite of what the
// Corn Plot app does with its CDN dependencies. Every cross-origin
// request this app makes is a weather query whose whole value is being
// current — caching an Open-Meteo response in the service worker would
// mean silently serving yesterday's accumulation with no way for the
// user to tell. Weather caching happens one layer up instead, in
// weather.js's localStorage cache, which is explicitly dated (it expires
// at the end of the calendar day it was fetched), is surfaced in the UI
// ("cached earlier today"), and can be cleared from Settings.

const CACHE_VERSION = "v1.3-beta";
